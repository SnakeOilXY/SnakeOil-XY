...
- Measure vibrations with the ADXL345 sensor mount on the toolhead. This step will generate 2 csv files (/tmp/resonances*x*???.csv and /tmp/resonances*y*???.csv)
- Measure the vibrations but with the ADXL345 sensor mount on the bed. This step will also generate 2 more csv files (/tmp/resonances*x*???.csv and /tmp/resonances*y*???.csv)
- Now you will have 4 csv files in <code>/tmp</code> directory, advance to the next step.

## 5. Calculate the shaper frequency

### 5.1. Method 1 : Use Klipper's calibrate_shaper with multy inputs

- Run the following commands, klipper will calculate the shaper frequency based on all 4 csv files we generated in the previous step.
...
### 5.2. Method 2 : Manually calculate the shaper frequency(if the above method does not work)
